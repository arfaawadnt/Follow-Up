...
/// <param name="Unplaced">Cash collections whose reps resolve to no serving branch, or to a branch no active treasury covers.</param>
public sealed record CollectionTreasurySyncResult(int Linked, int Unplaced);

/// <summary>
/// Mirrors every cash collection that has no treasury entry yet into the treasury serving its reps' branch (as a
/// Pending entry). The collection write handlers do this synchronously; this run covers collections recorded before
/// the mirroring existed and reps whose branch gained a treasury later. Implemented in Infrastructure; invoked from the
