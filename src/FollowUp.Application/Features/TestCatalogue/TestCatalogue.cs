...
/// <summary>Live "No-Lab Tests" fetch (from the Oracle NoLabTests feed). Shared by the report handler and the
/// daily statistics-email runner (a background job that cannot send the auth-gated query).</summary>
public interface INoLabTestsQueries
{
    Task<IReadOnlyList<NoLabTestRowDto>> ListAsync(DateOnly from, DateOnly to, CancellationToken ct);
}
...
{
    private readonly INoLabTestsQueries _queries;
    public GetNoLabTestsReportHandler(INoLabTestsQueries queries) => _queries = queries;
    public Task<IReadOnlyList<NoLabTestRowDto>> Handle(GetNoLabTestsReportQuery r, CancellationToken ct) =>
        _queries.ListAsync(r.From, r.To, ct);
}
