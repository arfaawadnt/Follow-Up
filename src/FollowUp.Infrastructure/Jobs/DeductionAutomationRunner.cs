...
internal sealed class DeductionAutomationRunner : IDeductionAutomationRunner
{
    ...
    public async Task<DeductionAutomationResult> RunAsync(DateOnly through, bool manual, CancellationToken ct)
    {
        ...
        _logger.LogInformation(
            "Deductions automation ({Mode}) {Month} through {Through:yyyy-MM-dd}: deal areas {DealAreas} (created {Created}, recalculated {Recalculated}, adjusted-skipped {Skipped}); penalties linked {Linked}, unplaced {Unplaced}.",
            mode, month, through, dealAreas.Count, created, recalculated, skipped, linked, unplaced);

        return new DeductionAutomationResult(month.ToString(), through, dealAreas.Count, created, recalculated, skipped, linked, unplaced);
    }
}
