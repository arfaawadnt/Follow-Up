# Deploy: Penalty Statement — "User Type" + "User" (the person who made the error) (feature/penalty-performed-by).
# -------------------------------------------------------------------------------------------------
# Ships the 4 managed DLLs + the Angular bundle AND applies 1 new EF migration on startup (MigrateAsync):
#   PenaltyRecordPerformedBy : penalty_record.performed_by_user_id / performed_by_rep_id (nullable, Restrict FKs to
#                              app_user / representative, indexed) + ck_penalty_record_performed_by (exactly the link
#                              matching penalty_user; legacy rows with neither link stay valid).
#   Domain        : PenaltyRecord.UserType (was User) + PerformedByUserId / PerformedByRepId with the matching invariant
#   Application   : Create/Update penalty carry UserType + the two ids (validated + scope-checked); GetPenaltyActorsQuery
#   Infrastructure: EF mapping + FKs; PenaltiesAsync resolves the person's name; PenaltyActorsAsync feeds the picker
#   Api           : PenaltyBody (userType, performedByUserId, performedByRepId); GET /accounting/penalty-actors
#   wwwroot       : Record-penalty dialog gains "User Type" + a dependent "User" picker; grid/export show "User Type / User"
#
...
if (-not (Select-String -Path "$srcBin\FollowUp.Infrastructure.dll" -Pattern 'PenaltyRecordPerformedBy' -Quiet)) { throw "ABORT: FollowUp.Infrastructure.dll lacks the PenaltyRecordPerformedBy migration - rebuild (pass -Build)." }
if (-not (Select-String -Path "$srcBin\FollowUp.Domain.dll" -Pattern 'PerformedByRepId' -Quiet)) { throw "ABORT: FollowUp.Domain.dll lacks PenaltyRecord.PerformedByRepId - rebuild (pass -Build)." }
$chunk = Get-ChildItem "$srcWeb\*.js" | Where-Object { (Get-Content $_.FullName -Raw) -match 'penalty-actors' } | Select-Object -First 1
if (-not $chunk) { throw "ABORT: no bundle file carries the penalty 'User' picker (penalty-actors) - rebuild (pass -Build)." }
Write-Host "Payload OK ($($chunk.Name) carries the new dialog)."
...
if (-not $SkipDbBackup) {
    ...
    $dumpFile = "C:\FollowUp\db-backup-penalty-performed-by-$stamp.dump"
...
# ---- App backup ----------------------------------------------------------------------------------------------
$backup = "C:\FollowUp\app-backup-penalty-performed-by-$stamp"
...
if ($healthy) {
    ...
    Write-Host "Verify (Ctrl+F5): Accounting -> Penalty Statement -> Record penalty: 'User Type' (Rep / Data Entry / Technician) and a"
    Write-Host "  dependent 'User' picker (reps for Rep, system users otherwise); the grid and exports show 'User Type / User'."
    Write-Host "  Existing rows show their type with a dash for the person until they are edited."
} else {
