...
public sealed class AdminSeedFailFastTests
{
    ...
    public async Task Seeding_the_admin_without_a_password_fails_fast()
    {
        ...
        await db.Database.ExecuteSqlRawAsync("DELETE FROM deduction; DELETE FROM penalty_record; DELETE FROM user_session; DELETE FROM app_user;" /* penalty_record RESTRICTs app_user (performed_by_user_id) */);
