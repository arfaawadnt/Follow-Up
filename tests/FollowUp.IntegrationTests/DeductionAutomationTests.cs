...
/// duplicated; penalties without a mirroring deduction get one; the DB refuses the shapes the domain forbids.
/// </summary>
...
public sealed class DeductionAutomationTests
{
    ...
    [SkippableFact]
    public async Task Daily_run_keeps_one_auto_deal_row_per_area_month_current_until_adjusted_and_links_unmirrored_penalties()
    {
        ...
        using (var scope = _fx.Services.CreateScope())
        {
            ...
            // A penalty written straight to the table (as records predating the automation were) — no mirror yet.
        ...
        try
        {
            ...
            using (var scope = _fx.Services.CreateScope())
            {
                ...
                r.PenaltiesLinked.Should().BeGreaterThanOrEqualTo(1);
                ...
                var mirror = rows.Should().ContainSingle(x => x.Origin == "AutoPenalty").Subject;
                mirror.PenaltyRecordId.Should().Be(legacyPenalty.Id.Value);
                mirror.PenaltySerial.Should().BeGreaterThan(0);
                mirror.Value.Should().Be(180m);
                mirror.SystemNote.Should().Contain("ACC-L");
            ...
            using (var scope = _fx.Services.CreateScope())
            {
                ...
                (await db.Deductions.CountAsync(d => d.PenaltyRecordId == legacyPenalty.Id)).Should().Be(1);
            }
...
VALUES ({Guid.NewGuid()}, {d10}, {area.Id.Value}, 'Transportation', 1, 'AutoPenalty', false, now(), 'test')");
                (await badOrigin.Should().ThrowAsync<Npgsql.PostgresException>()).Which.SqlState.Should().Be("23514", "AutoPenalty needs a penalty link");
...
INSERT INTO deduction (id, date, area_id, reason, ""value"", origin, is_adjusted, penalty_record_id, created_at, created_by)
VALUES ({Guid.NewGuid()}, {d10}, {area.Id.Value}, 'Penalty', 1, 'AutoPenalty', false, {legacyPenalty.Id.Value}, now(), 'test')");
                (await dupMirror.Should().ThrowAsync<Npgsql.PostgresException>()).Which.SqlState.Should().Be("23505", "one mirror per penalty");
            }
        }
        finally
        {
