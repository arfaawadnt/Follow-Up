# Deploy: Daily Follow-up / Transfer Management / Lab Check-in now show archived (past-day) records read-only.
# -------------------------------------------------------------------------------------------------
# Ships the 4 managed DLLs + the Angular bundle. NO migration, NO Oracle feed.
#   Application   : BoardItemDto/TransferItemDto/ReceivingItemDto gain an `Archived` flag
#   Infrastructure: OperationsQueries GetBoardAsync/GetTransferableAsync/GetAwaitingReceiptAsync now UNION the live
#                     daily_visit rows with the archived visit_history rows for the requested date range (mirrors the
#                     Sample-Tracking lifecycle query); archived rows are flagged Archived=true
#   wwwroot       : the three pages render archived rows read-only (actions/inputs hidden, "History" badge)
#
# Run in an ELEVATED PowerShell (service stop/start needs admin). Assumes Release DLLs + Angular bundle are already
# built this session; pass -Build to build them here.
# -------------------------------------------------------------------------------------------------
param([switch]$Build)
...
$backup = "C:\FollowUp\app-backup-history-pages-$stamp"
...
try {
    ...
    Write-Host "Verify: open Daily Follow-up / Transfers / Lab Check-in, set the date range to a past day -> archived rows appear with a 'History' badge and no action controls."
} catch {
