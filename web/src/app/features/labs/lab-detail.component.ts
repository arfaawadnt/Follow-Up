  ...
  template: `
    ...
    @if (lab(); as l) {
      ...
      <div class="foot-actions">
        <button type="button" class="btn btn-p" [disabled]="busy() || !auth.has('UpdateLabs')" (click)="save()">{{ 'save_changes' | t : 'Save Changes' }}</button>
    ...
    this.api.get<LabDetail>(`/labs/${this.id}`).subscribe({
      next: (l) => {
        ...
        this.f = {
          ...
          payer: l.payer ?? '', contractType: l.contractType ?? '', marketingRepId: l.marketingRepId ?? '', responsibleRepId: l.responsibleRepId ?? '',
