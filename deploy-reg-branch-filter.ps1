# Deploy: Lab Statistics + Test Statistics - "Reg Branch" multi-select filter (feature/real-income-sheet).
# -------------------------------------------------------------------------------------------------
# Ships the 4 managed DLLs + the Angular bundle AND applies the new EF migration on startup (MigrateAsync):
#   DailyLabStatisticRegBranch : daily_lab_statistic gains branch (registration branch code, default "") and its unique key
#                                becomes (date, lab_code, branch); a plain (date, lab_code) index stays. Additive - existing
#                                rows keep branch "" until their window is re-synced from Oracle.
#   Oracle        : the LabStats feed now groups by reg.branch_code as well (the stored allow-listed SQL is refreshed on startup).
#   Domain        : DailyLabStatistic.Branch (+ For(date, code, branch), NormalizeBranch)
#   Application   : LabStatDto.RegBranch (name via the Branches reference, else the code; null when unknown); xlsx import feeds
#                   the branch-less row
#   Infrastructure: sync upserts by (date, lab, branch) and drops stale keys per window; lab-stats read model names the branch
#   wwwroot       : Lab Statistics - new "Reg branch" multi-select beside "Serving branch" (visits KPI counts distinct lab-days);
#                   Test Statistics - the branch filter is relabelled "Reg branch" (it always was the registration branch)
#
# TO KNOW AFTER THIS RELEASE:
#   - Every daily_lab_statistic row synced BEFORE this release carries no branch, so the new filter shows nothing for them.
#     Re-sync the history you need from Lab Statistics -> "Sync from Oracle" (from/to popup); each re-synced window is
#     replaced by branch-split rows. The nightly 00:05 job covers every day from now on.
#   - Lab-day totals everywhere (Rep Statement, Rep Income LDM column, Deductions, Area Statistics, e-mail reports) sum
#     the branch rows and are unchanged.
#
...
if (-not (Select-String -Path "$srcBin\FollowUp.Infrastructure.dll" -Pattern 'DailyLabStatisticRegBranch' -Quiet)) { throw "ABORT: FollowUp.Infrastructure.dll lacks the DailyLabStatisticRegBranch migration - rebuild (pass -Build)." }
...
if (-not $SkipDbBackup) {
    ...
    $dumpFile = "C:\FollowUp\db-backup-reg-branch-$stamp.dump"
...
# ---- App backup ----------------------------------------------------------------------------------------------
$backup = "C:\FollowUp\app-backup-reg-branch-$stamp"
...
if ($healthy) {
    ...
    Write-Host "  1. Statistics -> Lab Statistics -> 'Sync from Oracle' for the history you want split by branch (e.g. 01/09 -> today)."
    Write-Host "  2. Lab Statistics: new 'Reg branch' multi-select (beside 'Serving branch'); Test Statistics: branch filter now reads 'Reg branch'."
    Write-Host "  3. Rep Statement / Rep Income LDM / Deductions totals are unchanged (they sum the branch rows)."
} else {
